 ...
 * 把从 idx 开始到栈顶的元素轮转n个位置. 对于n为正数时，轮转方向是向栈顶的;
 * 当 n 为负数时，向栈底方向轮转-n个位置. n的绝对值不可以比参于轮转的切片长度大.
*/
...
LUA_API const char *lua_tolstring (lua_State *L, int idx, size_t *len) {
  ...
  if (!ttisstring(o)) {
    if (!cvt2str(o)) {  /* 是否能转换为string */
      if (len != NULL) *len = 0;
      return NULL;
    }
    lua_lock(L);  /* 'luaO_tostring'函数会创建一个新的string */
    luaO_tostring(L, o);
    luaC_checkGC(L);
    o = index2value(L, idx);  /* previous call may reallocate the stack(前一个调用会重新分配堆栈) */
    lua_unlock(L);
  }
  if (len != NULL)
    *len = vslen(o);
