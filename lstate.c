...
LUA_API lua_State *lua_newstate (lua_Alloc f, void *ud) {
  ...
  preinit_thread(L, g);
  g->allgc = obj2gco(L);  /* by now, only object is the main thread 当前只有一个对象是主线程 */
  L->next = NULL;
  g->frealloc = f;
  g->ud = ud;
  g->warnf = NULL;
  ...
  g->gcrunning = 0;  /* no GC while building state 构建state时不会GC */
